...
import SPIRITS, { RARITY_COLORS, RARITY_NAMES, rollSpiritItemRarity, getSpiritsByRarity } from '../data/spirits.js';
...
import { getComboBonus } from '../data/balance-config.js';
...
export default class CandyMatch {
  constructor(container, options = {}) {
    ...
    // 인벤토리 수집 추적 (이모지별 개수)
    this._inventoryItems = {};    // { emoji: count }
    this._inventoryGold = 0;
    this._invFlashTimer = 0;      // 아이템 획득 시 반짝임
    ...
    switch (drop.type) {
      ...
      case 'spirit_part': {
        ...
        if (spirits.length > 0) {
          ...
          this._addToInventory(spirit.emoji);
        }
        break;
      }
      case 'spirit': {
  ...
  _isSpecial(r, c) {
    return this._isEquipmentSlot(r, c) || this._isHeroSlot(r, c) || this._isPetSlot(r, c);
  }
  ...
  _render() {
    ...
    const gap = 1;
    const cellSize = Math.floor((APP_W - (this.cols - 1) * gap) / this.cols);
    ...
    // 인벤토리 슬롯 HTML 생성
    const invEntries = Object.entries(this._inventoryItems);
    const maxInvSlots = 8;
    let invSlotsHtml = '';
    for (let i = 0; i < maxInvSlots; i++) {
      if (i < invEntries.length) {
        const [emoji, count] = invEntries[i];
        invSlotsHtml += `<span class="bp-inv-slot slot-filled" data-inv-idx="${i}">${emoji}<span class="bp-inv-count">${count > 1 ? count : ''}</span></span>`;
      } else {
        invSlotsHtml += `<span class="bp-inv-slot">·</span>`;
      }
    }
    // 골드 + 세트 + 조각 진행 요약
    const fragDots = Array.from({length:6}, (_,i) => i < this._fragmentProgress ? '●' : '○').join('');
    const invSummary = `<span class="bp-inv-summary">💰${GameState.gold} 📦${this._fragmentSets} <span class="bp-inv-frag">${fragDots}</span></span>`;
...

    {
      ...
      // 장비 슬롯 (6부위)
      const EQUIP_PARTS = [
        { key:'head', label:'머리', empty:'👤' },
        { key:'body', label:'몸통', empty:'👕' },
        { key:'arms', label:'팔', empty:'🛡' },
        { key:'wings', label:'날개', empty:'🦋' },
        { key:'legs', label:'다리', empty:'🦵' },
        { key:'shoes', label:'신발', empty:'👢' },
      ];
      const eq = GameState.equipped || {};
      const equipHtml = EQUIP_PARTS.map(p => {
        const item = eq[p.key];
        const emoji = item ? (item.emoji || '✨') : p.empty;
        const filled = item ? ' slot-filled' : '';
        const rColor = item && item.rarityColor ? `border-color:${item.rarityColor};` : '';
        return `<span class="bp-slot${filled}" style="${rColor}" title="${p.label}">${emoji}</span>`;
      }).join('');

      // 영웅 슬롯 (5) + 펫 (1)
      const heroSlots = GameState.heroSlots || [];
      const heroHtml = Array.from({length:5}, (_,i) => {
        const h = heroSlots[i];
        return h
          ? `<span class="bp-slot slot-filled" title="${h.name || '영웅'}">${h.emoji || '⚔️'}</span>`
          : `<span class="bp-slot" title="빈 슬롯">·</span>`;
      }).join('');
      const pet = GameState.petSlot;
      const petHtml = pet
        ? `<span class="bp-slot slot-filled slot-pet" title="${pet.name || '펫'}">${pet.emoji || '🐾'}</span>`
        : `<span class="bp-slot slot-pet" title="펫 없음">·</span>`;
      ...
      this.container.innerHTML = `
        <div class="candy-scene candy-compact">
          ...
          <div class="candy-compact-info" id="candy-info">
            ...
            <span class="cci-frag">🎒${Object.values(this._inventoryItems).reduce((a,b)=>a+b,0)}</span>
          ...
          <div class="bottom-panel">
            <div class="bp-section">
              <span class="bp-label">파티</span>
              <div class="bp-row">${heroHtml}<span class="bp-divider"></span>${petHtml}</div>
            </div>
            <div class="bp-section">
              <span class="bp-label">장비</span>
              <div class="bp-row">${equipHtml}</div>
            </div>
          </div>
    ...
    if (cleared) {
      this.container.querySelector('#candy-clear-btn').onclick = () => {
        this._cleanup();
        this.onComplete({ score: this.score, combo: this.totalCombo });
      };
    }
  ...
  _createFloatingOrb(x, y, gemIdx, isSpecial) {
    ...
    document.getElementById('app').appendChild(orb);
  ...
  async _resolveMatches() {
    ...
    while (true) {
      ...
      // 매치된 타일 수의 절반만큼 콤보 증가 (3매치+5매치=8타일 → /2=4콤보)
      this.comboCount += Math.floor(matchedKeys.length / 2);
      ...
      for (let mg = 0; mg < matchGroups.length; mg++) {
        ...
        if (this._fragmentProgress >= 6) {
          ...
          // ★ 조각 세트 완성 시 정령 파츠 아이템 실제 생성 → 인벤토리에 추가
          ...
          if (fragSpirits && fragSpirits.length > 0) {
            ...
            if (fragSpirit) {
              ...
              this._addToInventory(fragSpirit.emoji);
            }
          }
        }
      }
      ...
      if (triggeredExtra.size > 0) {
        this.comboCount += Math.floor(triggeredExtra.size / 2);
      ...
      if (bestTier && bestTier.ability && bestTier.ability.type === 'purify') {
        ...
        if (purgeExtra.size > 0) {
          this.comboCount += Math.floor(purgeExtra.size / 2);
        ...
        <div style="display:flex;gap:12px;justify-content:center;">
          ${items.map(it => `
            <div style="text-align:center;">
              <div style="font-size:var(--icon-lg);">${it.emoji}</div>
              <div style="font-size:var(--label-xs);color:var(--text-secondary);">${it.partEmoji || ''} ${it.part || ''}</div>
              <div style="font-size:var(--label-xs);color:${it.rarityColor};font-weight:700;">${it.rarityName}</div>
            </div>
          `).join('')}
        </div>
      `;
  ...
  // --- 인벤토리에 아이템 추가 (이모지 + 개수 추적) ---

  _addToInventory(emoji, count = 1) {
    this._inventoryItems[emoji] = (this._inventoryItems[emoji] || 0) + count;
    this._invFlashTimer = Date.now();
  }

  // --- Fly-to-Inventory Animation (하단 인벤토리 패널로 이동) ---

  _animateFlyToInventory(emoji, fromEl, fromX, fromY) {
    // 인벤토리 개수 추가
    this._addToInventory(emoji);
    ...
    if (fromEl) {
      ...
      startX = fromX || (APP_LEFT + APP_W / 2);
      startY = fromY || (APP_TOP + APP_H / 2);
    }
    ...
    document.getElementById('app').appendChild(flyEl);

    // 타겟: 하단 인벤토리 패널
    const invPanel = this.container.querySelector('#bp-inventory');
    let targetX, targetY;
    if (invPanel) {
      const invRect = invPanel.getBoundingClientRect();
      targetX = invRect.left + invRect.width / 2;
      targetY = invRect.top + invRect.height / 2;
    } else {
      targetX = APP_LEFT + APP_W / 2;
      targetY = APP_TOP + APP_H - 30;
    }
    ...
    // 도착 시 인벤토리 반짝임
    setTimeout(() => {
      if (invPanel) invPanel.classList.add('bp-inv-flash');
      setTimeout(() => { if (invPanel) invPanel.classList.remove('bp-inv-flash'); }, 400);
    }, 600);
  ...
  _renderHeroSlotCell(cell, r, c) {
    ...
    if (!unlocked) {
      ...
      cell.innerHTML = `
        ...
        <span class="hero-slot-name" style="font-size:clamp(7px,2vw,11px);color:${rarityColor};position:absolute;bottom:1px;left:50%;transform:translateX(-50%);white-space:nowrap;">${hero.name}</span>
      ...
      cell.innerHTML = `<span class="hero-slot-empty">⚔️</span><span style="font-size:clamp(7px,2vw,11px);color:#9b8aff;position:absolute;bottom:2px;left:50%;transform:translateX(-50%);">영웅${slotNum}</span>`;
  ...
  _showItemDetailPopup(item) {
    ...
    popup.innerHTML = `
      <div class="slot-detail-header" style="border-bottom:2px solid ${rarityColor};">
        <span style="font-size:var(--icon-lg);">${item.emoji}</span>
        <div>
          <div style="font-weight:700;font-size:var(--label-lg);">${item.name}</div>
          <div style="font-size:var(--label-sm);color:${rarityColor};">${RARITY_NAMES[item.rarity] || '일반'} ${item.slot ? '(' + item.slot + ')' : ''}</div>
        </div>
      </div>
      ${item.stats ? `<div class="slot-detail-stats" style="font-size:var(--label-md);">
        ${Object.entries(item.stats).map(([k, v]) => `<span>${k}: ${v > 0 ? '+' : ''}${v}</span>`).join(' ')}
      </div>` : ''}
      <button class="btn btn-sm" id="slot-popup-close" style="margin-top:clamp(6px,2vw,10px);font-size:var(--label-md);">닫기</button>
    `;
    document.getElementById('app').appendChild(popup);
  ...
  _showHeroDetailPopup(hero, slotIdx) {
    ...
    if (isRestricted) {
      ...
      if (sameRarity.length > 0) {
        occupiedList = `<div class="dup-occupied-list">
          <div style="font-size:var(--label-xs);color:var(--text-muted);margin-bottom:3px;">같은 등급 장착 중:</div>
          ${sameRarity.map(h => {
    ...
    popup.innerHTML = `
      <div class="slot-detail-header" style="border-bottom:2px solid ${rarityColor};">
        <span style="font-size:var(--icon-lg);" class="hero-fairy">${hero.emoji}</span>
        <div>
          <div style="font-weight:700;font-size:var(--label-lg);">${hero.name}</div>
          <div style="font-size:var(--label-sm);color:${rarityColor};">${RARITY_NAMES[hero.rarity] || '일반'} · ${attrEmoji} ${attrName}</div>
        </div>
      </div>
      ${dupInfo}
      ${occupiedList}
      ${hero.passiveSkill ? `<div class="slot-detail-skill">
        <div style="color:var(--gold);font-size:var(--label-sm);font-weight:600;">패시브: ${hero.passiveSkill.name}</div>
        <div style="font-size:var(--label-xs);color:var(--text-secondary);">${hero.passiveSkill.description}</div>
      </div>` : ''}
      ${hero.activeSkill ? `<div class="slot-detail-skill">
        <div style="color:var(--pink);font-size:var(--label-sm);font-weight:600;">액티브: ${hero.activeSkill.name}</div>
        <div style="font-size:var(--label-xs);color:var(--text-secondary);">${hero.activeSkill.description}</div>
      </div>` : ''}
      <button class="btn btn-sm touch-btn" id="slot-popup-close" style="margin-top:clamp(6px,2vw,10px);font-size:var(--label-md);"><span class="touch-icon">👆</span> 닫기</button>
    `;
    document.getElementById('app').appendChild(popup);
    popup.querySelector('#slot-popup-close').onclick = () => popup.remove();
      ...
      <div class="dup-warning-body">
        <div class="dup-warning-row">
          <span class="dup-warning-hero" style="border-color:${rarityColor};">
            ${hero.emoji}<br><span style="font-size:var(--label-xs);">${hero.name}</span>
          </span>
          <span class="dup-warning-vs">⚡</span>
          <span class="dup-warning-hero" style="border-color:${rarityColor};">
            ${conflictHero.emoji}<br><span style="font-size:var(--label-xs);">${conflictHero.name}</span>
          </span>
        </div>
  ...
  async _autoDetonateSpecialTile(tier, group, matchedSet) {
    ...
    if (extraCells.size > 0) {
      this.comboCount += Math.floor(extraCells.size / 2);
  ...
  async _dropSpiritPartsFromMatch(tier, group) {
    ...
    // 매치 중심점 찾기
    const wrapper = this.container.querySelector('#candy-board-wrapper');
    let startX = APP_LEFT + APP_W / 2;
    let startY = APP_TOP + APP_H / 2;
    ...
    // 드랍 연출: 매치 위치에서 떠오른 후 하단 인벤토리로 이동
    const invPanel = this.container.querySelector('#bp-inventory');
    let invTargetX = APP_LEFT + APP_W / 2;
    let invTargetY = APP_TOP + APP_H - 30;
    if (invPanel) {
      const invRect = invPanel.getBoundingClientRect();
      invTargetX = invRect.left + invRect.width / 2;
      invTargetY = invRect.top + invRect.height / 2;
    }

    for (let i = 0; i < items.length; i++) {
      const item = items[i];
      // 인벤토리에 개수 추가
      this._addToInventory(item.emoji);
      ...
      document.getElementById('app').appendChild(dropEl);
      ...
      // Phase 2: 하단 인벤토리로 이동
      const elLeft = parseFloat(dropEl.style.left);
      const elTop = parseFloat(dropEl.style.top);
      setTimeout(() => {
        if (dropEl.parentNode) {
          dropEl.classList.remove('match-drop-float');
          dropEl.classList.add('match-drop-fly');
          dropEl.style.transform = `translate(${invTargetX - elLeft}px, ${invTargetY - elTop}px) scale(0.3)`;
      ...
      setTimeout(() => {
        if (invPanel) invPanel.classList.add('bp-inv-flash');
        setTimeout(() => { if (invPanel) invPanel.classList.remove('bp-inv-flash'); }, 400);
      }, MATCH_SPIRIT_DROP.dropAnimation.floatUpDuration + 200);
  ...
  _triggerRageSkill() {
    ...
    const startRect = { left: centerX - 20, top: centerY - 20, width: 40, height: 40 };
    ...
    document.getElementById('app').appendChild(heroFly);
