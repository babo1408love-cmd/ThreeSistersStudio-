 ...
 * @param {number}  options.width       - 맵 너비 타일 수 (기본 25)
 * @param {number}  options.arenaWidth  - 배틀아레나 폭 타일 (기본 50 = 2000px)
 * @param {number}  options.arenaHeight - 배틀아레나 높이 타일 (기본 100 = 4000px)
...
export function generateSurvivorMap(options = {}) {
  ...
  // ── 맵 크기 계산 (종스크롤: 가로=화면폭, 세로 길게) ──
  const scrollDist = _calcScrollDistance(baseSpeed, accel, duration) * 0.5; // 거리 절반
  const bufferPx = 2000; // 화면높이 + 여유

  // 맵 폭 = 화면 폭 (스마트폰 크기, 좌우 스크롤 없음)
  const screenW = (typeof APP_W !== 'undefined' ? APP_W : 390);
  const fieldTilesW = options.width || Math.ceil(screenW / tileSize);
  const arenaTilesW = fieldTilesW;  // 아레나도 같은 폭
  const arenaTilesH = options.arenaHeight || 100;  // 배틀아레나 100타일 = 4000px (세로)

  // 세로 스크롤 거리 → 필드 높이
  const fieldTilesH = Math.ceil((scrollDist + bufferPx) / tileSize);
  const totalHeight = fieldTilesH + arenaTilesH;
  const width = fieldTilesW;

  const mapW = width * tileSize;
  const mapH = totalHeight * tileSize;

  // ── 배틀 아레나 영역 (맵 상단) ──
  const arenaStartY = arenaTilesH * tileSize;  // 아레나 끝 Y (아레나는 0 ~ arenaStartY)
  const arenaCenterX = mapW / 2;
  const arenaCenterY = (arenaTilesH * tileSize) / 2;
  ...
  // ── 오브젝트 배치 (행 기반 공간 분할) ──
  const BUCKET_ROWS = 10; // 버킷 하나당 10타일 (행 기준)
  const bucketCount = Math.ceil(totalHeight / BUCKET_ROWS);
  ...
  // 샘플링 간격: 대형 맵에서 성능 보장
  const sampleStep = Math.max(1, Math.floor(totalHeight / 600));

  for (const objDef of theme.objects) {
    for (let y = 0; y < totalHeight; y += sampleStep) {
      for (let x = 0; x < width; x++) {
        const key = `${x},${y}`;
        if (occupied.has(key)) continue;
        ...
        if (n < objDef.freq * sampleStep) {
          ...
          // 행 기반 버킷에 등록
          const bi = Math.min(bucketCount - 1, Math.floor(y / BUCKET_ROWS));
  ...
  const arenaDecorations = [
    { emoji: '🏛️', freq: 0.015, solid: true, w: 50, h: 60 },
    { emoji: '🔥', freq: 0.02,  solid: false, w: 20, h: 25 },
    { emoji: '⚔️', freq: 0.008, solid: false, w: 25, h: 25 },
  ];
  // 아레나 경계 장식 (가장자리에만 배치, 맵 상단)
  for (const decor of arenaDecorations) {
    for (let y = 0; y < arenaTilesH; y += 3) {
      for (let x = 0; x < width; x++) {
        const isEdge = x < 2 || x >= width - 2 || y < 2 || y >= arenaTilesH - 2;
        if (!isEdge) continue;
  ...
  // ── 시냇물 (필드 영역만, 세로 흐름, 아레나 아래쪽) ──
  let stream = null;
  if (theme.water) {
    const sx = Math.floor(width * 0.4) * tileSize;
    stream = { color: theme.water.color, x: sx, width: theme.water.width, points: [] };
    for (let y = arenaTilesH; y < totalHeight; y += 2) {
      const wobble = Math.sin(y * 0.3 + seed * 0.01) * tileSize;
      stream.points.push({ x: sx + wobble, y: y * tileSize });
    }
  }
  ...
  if (theme.particles) {
    ...
    for (let i = 0; i < count; i++) {
      ambientParticles.push({
        x: Math.random() * mapW,
        y: mapH - Math.random() * 1200, // 초기 화면 영역 (하단, 플레이어 시작 근처)
        vx: (Math.random() - 0.5) * theme.particles.speed,
        vy: theme.particles.type === 'snow' ? theme.particles.speed :
            theme.particles.type === 'ember' ? -theme.particles.speed * 0.5 :
            (Math.random() - 0.5) * theme.particles.speed,
  ...
  // ── 게이트 포인트 (필드→아레나 전환 지점, 상단) ──
  const gatePosition = {
    x: mapW / 2,
    y: arenaStartY + tileSize * 2,
  };

  // ── 스폰 포인트 (하단 중앙 시작, 아래→위 진행) ──
  const spawnPoints = {
    player: { x: mapW / 2, y: mapH - tileSize * 3 },
    enemies: [
      { x: mapW * 0.3, y: mapH * 0.8 },
      { x: mapW * 0.7, y: mapH * 0.7 },
  ...
  return {
    themeId, theme, width, height: totalHeight, tileSize,
    mapW, mapH, floor,
    objects: allObjects,
    objectBuckets, bucketRows: BUCKET_ROWS,
    stream, ambientParticles, spawnPoints, seed,
    // 서바이벌 전용 필드
    survivorMode: true,
    fieldHeight: fieldTilesH,    // 필드 타일 수 (세로)
    fieldHeightPx: fieldTilesH * tileSize,
    // 하위 호환: 기존 필드명도 유지
    fieldWidth: width,
    fieldWidthPx: width * tileSize,
    arenaStartY,
    arenaStartX: 0,             // 하위 호환 (아레나는 전체 가로폭)
...
export function generateAerialMap(options = {}) {
  ...
  return generateSurvivorMap({
    ...
    arenaWidth: options.arenaWidth || 50,
    arenaHeight: options.arenaHeight || 100,
    seed: options.seed || Date.now(),
  });
}

/**
 * 서바이벌 맵 렌더링 (행 기반 최적화, 종스크롤)
 * — 보이는 행의 오브젝트만 렌더링
 * — 배틀아레나 영역 시각적 강조 (하단)
 * — 앰비언트 파티클 카메라 리스폰
 */
export function renderSurvivorMap(ctx, map, camera) {
  const { theme, floor, objectBuckets, tileSize, mapW, mapH,
    arenaStartY, arenaWidth, arenaHeight,
    fieldHeightPx } = map;
  const bucketRows = map.bucketRows || map.bucketCols || 10; // 하위 호환

  const stream = map.stream;
  const ambientParticles = map.ambientParticles;
  ...
  // ── 배틀아레나 경계선 (가로선, 맵 상단) ──
  if (arenaStartY !== undefined) {
    const arenaBorderY = arenaStartY - cy;
    if (arenaBorderY > -100 && arenaBorderY < vh + 100) {
      // 가로 경계선 (점선)
      ctx.strokeStyle = 'rgba(255,215,0,0.3)';
      ctx.lineWidth = 3;
      ctx.setLineDash([12, 8]);
      ctx.beginPath();
      ctx.moveTo(0, arenaBorderY);
      ctx.lineTo(vw, arenaBorderY);
      ctx.stroke();
      ctx.setLineDash([]);

      // 게이트 표시
      const gateX = mapW / 2 - cx;
      if (gateX > -50 && gateX < vw + 50) {
        ctx.font = '32px serif';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText('🚪', gateX, arenaBorderY);
      }
    }

    // ── 아레나 영역 배경 오버레이 (0 ~ arenaStartY) ──
    const arenaScreenT = Math.max(0, 0 - cy);
    const arenaScreenB = Math.min(vh, arenaStartY - cy);
    if (arenaScreenB > 0 && arenaScreenT < vh) {
      ctx.fillStyle = 'rgba(255,180,0,0.03)';
      ctx.fillRect(0, arenaScreenT, vw, arenaScreenB - arenaScreenT);
    }
  }

  // ── 시냇물 (필드 영역만, 세로 흐름, 보이는 부분만) ──
  if (stream && stream.points.length > 1) {
    const streamStartIdx = Math.max(0, Math.floor((cy - 100) / (tileSize * 2)));
    const streamEndIdx = Math.min(stream.points.length, Math.ceil((cy + vh + 100) / (tileSize * 2)));
    ...
    if (streamEndIdx > streamStartIdx) {
      ...
      for (let i = streamStartIdx; i < streamEndIdx; i += 2) {
        const p = stream.points[i];
        const sx = p.x - cx;
        const sy = p.y - cy + Math.sin(t + i) * 5;
        ctx.beginPath();
        ctx.moveTo(sx, sy - 5);
        ctx.lineTo(sx, sy + 5);
        ctx.stroke();
  ...
  // ── 오브젝트 (행 기반 버킷 — 보이는 버킷만) ──
  const bucketTilePx = bucketRows * tileSize;
  const startBucket = Math.max(0, Math.floor((cy - 60) / bucketTilePx));
  const endBucket = Math.min(objectBuckets.length, Math.ceil((cy + vh + 60) / bucketTilePx));
  ...
  for (const p of ambientParticles) {
    ...
    // 카메라 주변 리스폰 (X축: 맵 폭 기준, Y축: 카메라 기준)
    if (p.x < 0) p.x = mapW;
    if (p.x > mapW) p.x = 0;
    if (p.y < cy - 100) p.y = cy + vh + Math.random() * 50;
    if (p.y > cy + vh + 100) p.y = cy - Math.random() * 50;
