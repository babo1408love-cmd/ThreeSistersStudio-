/**
 * enemy-drop-generator.js — 적/드롭 생성기
 * 적 10종+보스 4종, 드롭: 골드/장비15종/소비7종
 * generateWave 웨이브 시스템
 * BalanceAI 자동 밸런싱 연동
 *
 * ※ 슬롯 영웅(동료)은 드롭 불가 — 소환의 방에서만 획득
 */
...
// ※ 동료(슬롯 영웅)는 드롭 불가 — 소환의 방에서만 배치
// 동료 데이터는 js/data/characters.js 및 hero-config.js 참조
...
export function generateDrop(enemyDef, waveNum = 1) {
  ...
  const goldAmount = typeof FormulaPack1 !== 'undefined'
    ? FormulaPack1.getGoldReward(goldBase, stageLevel)
    : Math.round(goldBase * (0.8 + Math.random() * 0.4));
  ...
  const equipRate = typeof FormulaPack1 !== 'undefined'
    ? FormulaPack1.getEquipDropRate(stageLevel, enemyDef.rarity || 'normal')
    : 0.10 + (rarityBonus[enemyDef.rarity]||0);
  ...
  // Consumable (15%)
  // if (Math.random() < 0.15) {
  const consumeRate = typeof FormulaPack1 !== 'undefined'
    ? Math.min(0.40, 0.15 + stageLevel * 0.002)
    : 0.15;
  if (Math.random() < consumeRate) {
...
export function generateBossWave(bossId, stageLevel = 1) {
  ...
  const scaling = typeof FormulaPack1 !== 'undefined'
    ? FormulaPack1.scaleEnemyStat(1, stageLevel, 'hp', 1)
    : 1 + (stageLevel - 1) * 0.15;
