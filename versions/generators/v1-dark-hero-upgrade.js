...
export function getRequiredExp(level, classId) {
  if (level >= MAX_LEVEL) return Infinity;
  if (typeof FormulaPack3 !== 'undefined') {
    return FormulaPack3.getRequiredEXP(level, classId || 'warrior');
  }
  // 기존 고정값 (폴백)
...
export class HeroManager {
  ...
  addExp(amount) {
    ...
    while (this.level < MAX_LEVEL) {
      const needed = getRequiredExp(this.level, this.heroClass);
  ...
  computeCombatStats(baseStats, equippedItems = {}) {
    ...
    // 레벨 성장
    if (typeof FormulaPack3 !== 'undefined') {
      const formulaStats = FormulaPack3.getAllStatsAtLevel(this.level, this.heroClass);
      stats.maxHp = formulaStats.hp;
      stats.attack = formulaStats.atk;
      stats.defense = formulaStats.def;
      stats.speed = formulaStats.spd;
    } else if (cls) {
      // 기존 고정값 (폴백)
      // for (const [stat, growth] of Object.entries(cls.growth)) {
