...
export default class RageSystem {
  ...
  constructor(options = {}) {
    ...
    if (typeof FormulaPack5 !== 'undefined') {
      const heroRarity = options.heroRarity || 1;
      const heroLevel = options.heroLevel || 1;
      this.damageMultiplier = FormulaPack5.getRageDamageMultiplier(heroRarity, heroLevel);
    } else {
      this.damageMultiplier = options.damageMultiplier || 2.0;
    }
  ...
  add(amount) {
    ...
    // 횟수 소진 후에도 충전 허용 — 소진 시 절반 효율
    const rate = this.triggerCount >= this.maxTriggers ? 0.5 : 1.0;

    this.gauge = Math.min(this.maxGauge, this.gauge + amount * this.gainRate * rate);
  ...
  trigger() {
    if (this.active) return false;
    // 횟수 소진 후에도 발동 가능 — 지속시간 절반 패널티
    const exhausted = this.triggerCount >= this.maxTriggers;

    this.active = true;
    this.gauge = 0;
    this.timer = exhausted ? Math.round(this.duration * 0.5) : this.duration;
    this.triggerCount++;
    return true;
  }
