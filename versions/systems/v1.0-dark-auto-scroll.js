...
export default class AutoScroll {
   ...
   * @param {string}  options.direction     - 'horizontal'(좌→우) | 'vertical'(하→상, 아래에서 추격) | 'radial'(중심 수축)
   * @param {number}  options.startBoundary - 시작 경계 위치 (vertical: mapHeight, horizontal: 0)
  ...
  update(dt, player) {
    ...
    if (this.direction === 'vertical') {
      // 종스크롤: 아래에서 추격 — player.y > boundary → 위험
      ...
      if (result.inDamage) {
        ...
        if (this._damageTimer >= 500) {
          ...
          if (typeof FormulaPack5 !== 'undefined') {
            const fogData = FormulaPack5.getSporeFogDamage(this._stageLevel || 1, this._elapsed / 1000);
            result.damage = Math.round(fogData.damagePerSec * 0.5);
          } else {
            result.damage = Math.round(this.damagePerSec * 0.5);
          }
        }
        result.pushY = -this.pushForce * (dt / 16); // 위로 밀어냄
      }

      if (result.inWarning) {
        const ratio = 1 - (this.boundary - pos) / this.warningZone;
        result.pushY = -this.pushForce * 0.3 * ratio * (dt / 16); // 위로 밀어냄
      }
    } else {
      // 횡스크롤: 왼쪽에서 추격 — player.x < boundary → 위험
      const warningEdge = this.boundary + this.warningZone;
      result.inDamage = pos < this.boundary;
      result.inWarning = pos < warningEdge && !result.inDamage;
      ...
      if (result.inDamage) {
        ...
        result.pushX = this.pushForce * (dt / 16);
      }

      if (result.inWarning) {
        const ratio = 1 - (pos - this.boundary) / this.warningZone;
        result.pushX = this.pushForce * 0.3 * ratio * (dt / 16);
      }
    }
  ...
  _drawVertical(ctx, camera, W, H) {
    // 종스크롤: 아래에서 올라오는 포자 안개
    const by = this.boundary - camera.y;

    if (by < H + 200) {
      // 짙은 안개 (하단에서 위로)
      const fogTop = Math.min(H, by - 80);
      const grad = ctx.createLinearGradient(0, H, 0, Math.max(0, fogTop));
      grad.addColorStop(0, 'rgba(30,0,40,0.95)');
      grad.addColorStop(0.6, 'rgba(50,10,60,0.8)');
      grad.addColorStop(0.85, 'rgba(80,20,100,0.4)');
      grad.addColorStop(1, 'rgba(100,30,120,0)');
      ctx.fillStyle = grad;
      ctx.fillRect(0, Math.max(0, fogTop), W, H - Math.max(0, fogTop));
      ...
      if (wy > 0 && wy < H) {
        const warnGrad = ctx.createLinearGradient(0, by, 0, wy);
        warnGrad.addColorStop(0, 'rgba(168,85,247,0.15)');
        warnGrad.addColorStop(1, 'rgba(168,85,247,0)');
        ctx.fillStyle = warnGrad;
        ctx.fillRect(0, Math.min(wy, by), W, Math.abs(by - wy));
      }
    }
