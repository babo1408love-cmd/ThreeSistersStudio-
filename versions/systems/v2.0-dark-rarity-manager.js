...
// ── 등급별 전체 스탯 가져오기 ──
export function getRarityStats(rarityId, level) {
  if (typeof FormulaPack4 !== 'undefined' && level) {
    const scaled = FormulaPack4.getAllRarityStats(rarityId, level);
    return {
      defense: scaled.def,
      atk: scaled.atk,
      atkSpeed: scaled.atkSpd,
      moveSpeed: scaled.moveSpd,
