...
const UnitFactory = {
  ...
  /**
   * 맨몸 전투력 (적 스케일링용)
   * 장비/슬롯영웅 보너스 제외 — 장비를 모을 인센티브 보장
   */
  getNakedPlayerPower() {
    // HeroCore가 있으면 EquipmentScalingAI 사용
    if (typeof window !== 'undefined' && window.HeroCore && window.HeroCore.getNakedPower) {
      return window.HeroCore.getNakedPower();
    }
    // 폴백: GameState.getBaseStats()
  ...
  createEnemy(baseDef, scaling = 1, options = {}) {
    ...
    // BalanceAI 검증 (선택적, 맨몸 전투력 기준)
    if (options.validateBalance) {
      const ai = this._getBalanceAI();
      const nakedPower = options.playerPower || this.getNakedPlayerPower();
      if (ai && nakedPower) {
        const enemyPower = ai.calcPower(enemy.hp, enemy.atk, enemy.def, baseDef.spd || 1);
        const check = ai.checkBalance(nakedPower, enemyPower);
  ...
  createWave(waveNum, stageLevel, playerPower, enemiesDict, bossesDict) {
    // 맨몸 전투력 사용 — 장비/슬롯영웅 보너스로 적이 강해지지 않음
    if (playerPower <= 0) playerPower = this.getNakedPlayerPower();
  ...
  createPlayerEntity(gameState, options = {}) {
    ...
    const mapW = options.mapW || 1000;
    const mapH = options.mapH || 4000;
    return {
      x: mapW / 2,
      y: mapH - 120,
