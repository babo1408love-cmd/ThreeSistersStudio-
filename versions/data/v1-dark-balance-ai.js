...
const BalanceAI = {
  ...
  // ========== 성장 공식 (하이브리드) ==========
  calcGrowth(baseStat, level, cap, monsterType) {
    // FormulaPack5 연동 (몬스터 타입별 차별 성장)
    if (typeof FormulaPack5 !== 'undefined' && monsterType) {
      const stats = FormulaPack5.getMonsterAllStats(
        { hp: baseStat, atk: baseStat, def: baseStat },
        level,
        monsterType
      );
      return stats.hp; // 해당 stat의 결과
    }
    // 기존 고정값 (폴백)
