...
class SurvivalEngine {
  constructor(canvas, options = {}) {
    ...
    // Player (맵 하단 중앙, 종스크롤: 아래→위 진행)
    const ps = GameState.player;
    this.player = {
      x: this.map.mapW / 2, y: this.map.mapH - 120,
    ...
    // 카메라: 플레이어 하단 20% (종스크롤: 아래→위, 첫 프레임부터 올바른 위치)
    this.camera = {
      x: Math.max(0, this.player.x - canvas.width * 0.5),
      y: Math.max(0, this.player.y - canvas.height * 0.8),
    };
    ...
    // 🌫️ 자동 전진 (포자 안개, 종스크롤: 아래에서 추격)
    this.autoScroll = new AutoScroll({
      speed: 0.5,
      direction: 'vertical',
      startBoundary: this.map.mapH,
      warningZone: 100,
      damagePerSec: 20,
      pushForce: 1.8,
      accel: 0.00006,
  ...
  _spawnWave() {
    ...
    const count = Math.min(8, Math.ceil((3 + this.currentWave) / 2));
  ...
  _spawnEnemy(def, scale, isElite) {
    // 종스크롤: 전방 양쪽에서 스폰 (아래→위 진행)
    const fromLeft = Math.random() < 0.5;
    const sideMargin = this.map.mapW * 0.15;
    const x = fromLeft
      ? Math.random() * sideMargin + 20
      : this.map.mapW - Math.random() * sideMargin - 20;
    const spawnMinY = this.player.y - this.H * 1.0;
    const spawnMaxY = this.player.y - this.H * 0.3;
  ...
  _updatePlayer(dt) {
    ...
    // 종스크롤: 아래→위 진행, 보스=상단 경계, 안개=하단 경계
  ...
  _updateCamera() {
    // 종스크롤: 플레이어를 화면 하단 20%에 배치 → 전방(위) 80% 시야 확보
    // X축 고정 (맵 폭 = 화면 폭, 좌우 스크롤 없음)
    this.camera.x = 0;
    const targetY = this.player.y - this.H * 0.8;
...
export default class SurvivalScene {
  ...
  _showBiomeSelect() {
    ...
    this.el.innerHTML = `
      ...
      <style>
        .survival-select { padding: clamp(8px,3vw,16px); max-width: 500px; margin: 0 auto; }
        .sv-header { display: flex; align-items: center; gap: 10px; margin-bottom: 12px; }
        .sv-header h2 { flex: 1; text-align: center; font-size: clamp(18px,5vw,22px); color: #f0e6d2; margin: 0; }
        .sv-desc { font-size: clamp(12px,3.2vw,15px); color: #888; text-align: center; margin-bottom: 16px; }
        .sv-biome-list { display: grid; grid-template-columns: repeat(2, 1fr); gap: 8px; }
        .sv-biome {
          display: flex; align-items: center; gap: 8px; padding: clamp(8px,3vw,14px);
        ...
        .sv-biome-emoji { font-size: clamp(20px,6vw,28px); }
        .sv-biome-name { font-size: clamp(12px,3.2vw,15px); color: #f0e6d2; }
      </style>
    `;
