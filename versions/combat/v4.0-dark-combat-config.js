...
/**
 * FormulaPack5 연동: 실시간 자동공격 간격
 * 기존 고정값: autoAttackInterval = 800
 */
export function getAutoAttackInterval(playerAtkSpeed, stageLevel) {
  // const interval = BATTLE_ARENA_CONFIG.autoAttackInterval; // 기존 800ms 고정
  if (typeof FormulaPack5 !== 'undefined') {
    return FormulaPack5.getAutoAttackInterval(playerAtkSpeed || 800, stageLevel || 1);
  }
  return BATTLE_ARENA_CONFIG.autoAttackInterval;
}
...
export const BOSS_ROOM_CONFIG = {
  ...
  // --- 보스방 공간 설계 (전맵 크기) ---
  arena: {
    // 전맵 규모 배틀아레나 (절반 — 차차 조정)
    width: 1000,               // 캔버스 논리 너비 (카메라 스크롤)
    height: 500,               // 캔버스 논리 높이 (카메라 스크롤)
    // 실제 이동 가능 영역 (벽 안쪽)
    playArea: {
      x: 60,
      y: 60,
  ...
  // ====================================================
  // 공중전 Phase 2 — 요정 보스의 분노 (비행기 공중전 스타일)
  // 보스 HP 50% 이하 → 공중전 전환
  // ====================================================
  aerialPhase2: {
    // 전환 연출
    transition: {
      duration: 2000,           // 2초 전환
      fadeOutDur: 500,          // 보스방 페이드아웃
      skySweepDur: 500,         // 카메라 하늘 스윕
      textDur: 500,             // 텍스트 표시
      spawnDur: 500,            // 보스/플레이어 배치
      text: '\u26A1 \uACF5\uC911\uC804 \uB3CC\uC785!',  // ⚡ 공중전 돌입!
    },
    ...
    boss: {
      scale: 2.5,               // 이모지 확대
      baseFontSize: 42,
      trackLerp: 0.02,          // X축 플레이어 추적 보간
      vertBounceAmp: 8,         // 수직 바운스 진폭 (작게)
      vertBounceFreq: 0.001,    // 수직 바운스 주파수
      enrageThreshold: 0.3,     // 30% HP → 분노 모드
      enrageShake: 4,           // 분노 흔들림 px
      yRange: [0.25, 0.39],     // 2단계 위로 올림 (1단계=0.04)
    },

    // 플레이어 설정
    player: {
      speedMult: 1.2,           // 회피용 속도 가속
      yRange: [0.55, 0.88],     // 맵 위에서 자유이동
      fontSize: 42,
    },
    ...
    // 보스 공격 패턴
    patterns: {
      cooldown: 2500,           // 일반 쿨다운 ms
      enrageCooldown: 1500,     // 분노 쿨다운 ms
      // 패턴별 설정
      sporeRain: {
        duration: 3000,
        count: [8, 15],         // [min, max]
      ...
      laserBeam: {
        ...
        beamWidth: 30,
        color: '#ff4444',
        warningColor: 'rgba(255,0,0,0.3)',
      },
      diveBomb: {
        duration: 2000,
        telegraphDur: 500,
        damageRatio: 0.6,
        diveTargetY: 0.55,      // 플레이어 영역 상단까지 돌진
        returnSpeed: 3,
      },
      summonMinions: {
        duration: 4000,
      ...
      barrage: {
        ...
        bulletRadius: 5,
        emoji: '\uD83D\uDCA0',  // 💠
        hpThreshold: 0.3,       // 30% 이하만
      },
    },

    // 패턴 가중치 (HP 비율별)
    patternWeights: {
      normal: { sporeRain: 30, laserBeam: 25, diveBomb: 20, summonMinions: 25, barrage: 0 },
      enraged: { sporeRain: 20, laserBeam: 20, diveBomb: 15, summonMinions: 15, barrage: 30 },
    },

    // 배경 설정
    background: {
      skyGradient: ['#050510', '#0a0a18', '#0e0816', '#1a0e24'],
      speedLineCount: 30,
      speedLineAlpha: [0.05, 0.18],
      speedLineSpeed: [10, 20],
      cloudCount: 8,
      cloudAlpha: [0.05, 0.12],
      cloudSpeed: [2, 4],
      cloudSizeRange: [25, 60],
      parallaxLayers: 3,
      parallaxSpeeds: [0.3, 0.6, 1.0],
      parallaxColors: ['#0a0a12', '#1a1a24', '#161620'],
    },
    ...
    hud: {
      bossHpBarY: 30,
      bossHpBarHeight: 24,
      bossHpBarWidthRatio: 0.6,
      playerHpBarX: 20,
      playerHpBarY: -40,        // 하단 기준 오프셋
      playerHpBarW: 150,
      playerHpBarH: 14,
      rageBarX: -170,           // 우측 기준 오프셋
      rageBarY: -40,
      rageBarW: 150,
      rageBarH: 14,
    },
...
// ====================================================
// 아레나 웨이브 설정 — 보스방 내 슬라임 웨이브 타이밍
// ====================================================
export const ARENA_WAVE_CONFIG = {
  waveInterval: 3000,       // 3초마다 슬라임 웨이브
  firstWaveDelay: 2000,     // 보스 ACTIVE 후 2초 뒤 첫 웨이브
  maxSlimes: 20,            // 동시 최대 슬라임
  useWavePhases: true,      // WAVE_PHASES spawnMult 연동
};
...
export const ENEMY_SPEED_CONFIG = {
  // 원거리 속도 배율 (플레이어 속도 대비) — ★ 2배 적용
  farSpeedMultiplier: 3.2,        // 원거리: 플레이어의 320% (빠르게 접근)
  // 근거리 속도 배율 (플레이어 속도 대비)
  nearSpeedMultiplier: 1.0,       // 근거리: 플레이어와 동일 속도
  ...
  // 보스 전용 오버라이드
  boss: {
    farSpeedMultiplier: 2.8,      // 보스 원거리: 280%
    nearSpeedMultiplier: 1.0,     // 보스 근거리: 플레이어와 동일 속도
    proximityRadius: 60,
    farRadius: 350,
  },
