...
import GoldenMimicBelly from './golden-mimic-belly.js';
import AerialBossPhase2 from './aerial-boss-phase2.js';
...
export default class CombatEngine {
  constructor(canvas, options = {}) {
    ...
    this._aerialTest = options.aerialTest || false;
    ...
    // 🛡️ 방패병 + GOLDMIMIC 풍선 시스템
    this._shieldSpawnCount = 0;
    this._goldmimicWord = ['G','O','L','D','M','I','M','C'];
    // 수집 상태: GameState에서 로드 (판 넘어서도 유지)
    this._collectedSlots = GameState.goldmimicSlots
      ? [...GameState.goldmimicSlots]
      : new Array(8).fill(false);
    this._collectedCount = this._collectedSlots.filter(Boolean).length;
    // 완성 연출 상태
    this._mimicCompleteAnim = null; // { phase, timer } — neon→giant→eat→belly
    this._mimicBelly = null;        // GoldenMimicBelly 인스턴스

    // 공중전 Phase 2 (보스 분노)
    this._aerialPhase2 = null;           // AerialBossPhase2 인스턴스
    this._combatPausedForAerial = false;
    ...
    this._perspMode = options.perspMode || 'high';
    ...
    this._bgScroll = 0;
    this._bgScrollSpeed = 0.025; // yNorm/초 (절반 속도, 40초 순환)
  ...
  start() {
    ...
    // ✈️ 공중전 Phase 2 테스트: low 시점 + 즉시 진입 (더미 보스 생성)
    if (this._aerialTest) {
      // 로우뷰 카메라 적용 (아래에서 올려다보기)
      this._perspMode = 'low';
      this._perspPreset = this.PERSP_PRESETS['low'];
      setTimeout(() => {
        ...
        const testBoss = {
          ...
          currentHp: 500,
          maxHp: 500,
          atk: 15, attack: 15, def: 5, defense: 5,
          speed: 1.5, size: 3, alive: true,
        };
        this._startAerialPhase2(testBoss, testBoss.maxHp);
      }, 300);
    }
  }
  ...
  // ══════════════════════════════════════
  _update(dt) {
    // 🛡️ 미믹 연출 or 뱃속 활성 → 전투 전체 스킵
    if (this._mimicCompleteAnim || (this._combatPausedForBelly && this._mimicBelly)) {
      this._updateMimicSequence(dt);
      if (this._combatPausedForBelly && this._mimicBelly) {
        this._mimicBelly._keys = this._keys;
        this._mimicBelly._touchDir = this._touchDir;
        this._mimicBelly.update(dt);
      }
      return;
    }

    // ✈️ 공중전 Phase 2 활성 → 전투 전체 스킵
    if (this._combatPausedForAerial && this._aerialPhase2) {
      this._aerialPhase2._keys = this._keys;
      this._aerialPhase2._touchDir = this._touchDir;
      this._aerialPhase2.update(dt);
      return;
    }
  ...
  _spawnFormation(mobData, count, cols) {
    ...
    // 대형 기준 Y — 화면 최상단 (카메라 기준 -5~5% 위치)
    this._formationRow++;
    const baseY = this.camera.y + this.H * (-0.05 + this._formationRow * 0.04);

    // 대형 간격 (원근에 따라 조절)
    const rowGap = 28 + (1 - pp.minDepth) * 8;
    const colGap = 30 + pp.minDepth * 10;

    // 길 양쪽 스폰 — 좌/우 교대
    if (this._spawnLane === undefined) this._spawnLane = 0;
    const lane = this._spawnLane;
    this._spawnLane ^= 1; // 다음번엔 반대쪽

    // 좌측 레인: 길 왼쪽 가장자리, 우측 레인: 길 오른쪽 가장자리
    const centerX = lane === 0
      ? mapW * 0.28 + (Math.random() - 0.5) * 20  // 좌측 길가
      : mapW * 0.72 + (Math.random() - 0.5) * 20; // 우측 길가
    ...
    // 🛡️ 방패병 스폰 트리거 — 10마리 스폰마다
    this._shieldSpawnCount += spawned;
    while (this._shieldSpawnCount >= 10) {
      this._shieldSpawnCount -= 10;
      this._spawnShieldMob();
    }
  }
  ...
  _createFormationUnit(mobData, sx, sy) {
    ...
    const def = {
      ...
      def: 0,
      spd: Math.round(mobData.spd * 5),  // 속도 절반 (×10 → ×5)
      color: visual.color,
      emoji: visual.emoji,
  ...
  _handleSurvivorEvent(event) {
    if (event.type === 'swarm_rush') {
      ...
      // 보스/미니보스: 화면 상단 중앙에서 등장
      const bx = this.map.mapW * 0.5 + (Math.random() - 0.5) * 60;
      const by = Math.max(50, this.camera.y + this.H * 0.08);
      const def = {
        id: `sb_${event.type}`,
        name: event.name,
        hp: event.hp,
        ...
        def: 0,
        spd: Math.round(event.spd * 5),  // 속도 절반
  ...
  /** 방패병 스폰 — HP 10배, ATK 0, 풍선에 8% 확률로 GOLDMIMC 글자 */
  _spawnShieldMob() {
    const baseHp = 60 + this.stageLevel * 20;
    const shieldHp = Math.round(baseHp * 10);
    ...
    const sy = this.player.y - this.H * 1.0;

    // 풍선 글자 판정: 8% 확률, 미수집 글자만 드롭
    let shieldLetter = null;
    let shieldLetterIdx = -1;
    let balloonDisplay = '🎈';
    if (Math.random() * 100 < 8) {
      // 아직 수집 안 된 슬롯만 후보
      const uncollected = [];
      for (let i = 0; i < this._goldmimicWord.length; i++) {
        if (!this._collectedSlots[i]) uncollected.push(i);
      }
      if (uncollected.length > 0) {
        shieldLetterIdx = uncollected[Math.floor(Math.random() * uncollected.length)];
        shieldLetter = this._goldmimicWord[shieldLetterIdx];
        balloonDisplay = shieldLetter;
      }
    }
    ...
    entity.isShieldMob = true;
    entity.passive = true;
    entity._shieldLetter = shieldLetter;
    entity._shieldLetterIdx = shieldLetterIdx;
    entity._balloonDisplay = balloonDisplay;
    entity._balloonPhase = Math.random() * Math.PI * 2;
    this.enemies.push(entity);
  }
  ...
  _updateCamera() {
    ...
    if (brs) {
      ...
      // 일반 필드: 종스크롤 (X 고정)
      // high 시점: 플레이어가 화면 93% 위치 (최대한 하단, 전방 시야 극대화)
      // normal/low: 플레이어가 화면 90% 위치 (기존)
      this.camera.x = 0;
      const camRatio = this._perspMode === 'high' ? 0.93 : 0.9;
      const targetY = this.player.y - this.H * camRatio;
      this.camera.y += (targetY - this.camera.y) * 0.08;
  ...
  _onEnemyDeath(enemy) {
    ...
    // 🛡️ 방패병 처치 보상 — 풍선 글자 수집 (중복 방지)
    if (enemy.isShieldMob) {
      const idx = enemy._shieldLetterIdx;
      if (enemy._shieldLetter && idx >= 0 && !this._collectedSlots[idx]) {
        this._collectedSlots[idx] = true;
        this._collectedCount++;
        // GameState에 저장 (판 넘어서도 유지)
        GameState.goldmimicSlots = [...this._collectedSlots];
        // 파란 글자 획득 파티클
        for (let i = 0; i < 12; i++) {
          this.particles.push({
            x: enemy.x, y: enemy.y,
            vx: (Math.random() - 0.5) * 4, vy: -Math.random() * 3,
            life: 900, color: i % 2 === 0 ? '#4488FF' : '#88CCFF',
            size: 3 + Math.random() * 2, type: 'circle',
          });
        }
        // 8글자 완성 → 네온사인 연출 시작
        if (this._collectedCount >= 8) {
          this._startMimicCompleteSequence();
        }
      } else {
        // 빈 풍선 또는 이미 수집된 글자 → 풍선 터지는 파티클
        for (let i = 0; i < 8; i++) {
          this.particles.push({
            x: enemy.x, y: enemy.y - 20,
            vx: (Math.random() - 0.5) * 3, vy: -Math.random() * 2,
            life: 600, color: i % 2 === 0 ? '#FF6B6B' : '#FFB366',
            size: 2 + Math.random() * 2, type: 'circle',
          });
        }
      }
      // 방패병도 일반 보상은 계속 진행 (return 안 함)
    }
  ...
  _draw() {
    ...
    // 🛡️ 미믹 완성 연출 (모든 것 위에 오버레이)
    this._drawMimicSequence(ctx);

    // ✈️ 공중전 Phase 2 (모든 것 덮어씌움)
    if (this._combatPausedForAerial && this._aerialPhase2) {
      this._aerialPhase2.draw(ctx);
    }

    // 🛡️ 미믹 뱃속 미니게임 (모든 것 덮어씌움)
    if (this._combatPausedForBelly && this._mimicBelly) {
      this._mimicBelly.draw(ctx);
    }
  }
  ...
  _drawSlime(ctx, e, cx, cy) {
    ...
    // 레어도 글로우 (희귀 이상, 미믹/방패병은 위에서 처리)
    if (!e.isMimic && !e.isShieldMob) {
      const rarityGlow = {
    ...
    if (e.isShieldMob) {
      // 방패 (몸체 앞)
      const shieldSize = Math.round(r * 0.9);
      ctx.font = `${shieldSize}px serif`;
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText('🛡️', sx + r * 0.55, sy + r * 0.1);

      // 파란 테두리
      const pulse = 0.5 + Math.sin(Date.now() * 0.004) * 0.5;
      ctx.strokeStyle = `rgba(68,136,255,${0.5 + pulse * 0.5})`;
      ctx.lineWidth = 2.5;
      ctx.shadowColor = '#4488FF';
      ...
      // 풍선 (머리 위 — 떠다니는 애니메이션)
      e._balloonPhase = (e._balloonPhase || 0) + 0.03;
      const balloonFloat = Math.sin(e._balloonPhase) * 4;
      const balloonY = sy - r * squishY - 18 + balloonFloat;
      const balloonX = sx + Math.sin(e._balloonPhase * 0.7) * 3;

      // 풍선 줄
      ctx.strokeStyle = 'rgba(180,180,180,0.6)';
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(sx, sy - r * squishY - 2);
      ctx.quadraticCurveTo(sx + Math.sin(e._balloonPhase * 0.7) * 2, balloonY + 10, balloonX, balloonY + 4);
      ctx.stroke();
      ...
      if (e._shieldLetter) {
        ...
        ctx.beginPath();
        ctx.ellipse(balloonX, balloonY, r * 0.45, r * 0.5, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText(e._balloonDisplay, balloonX, balloonY);
      } else {
        // 빈 풍선 — 빨간 풍선 이모지
        ctx.font = `${Math.round(r * 0.6)}px serif`;
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText('🎈', balloonX, balloonY);
      }
    }
    ...
    if (e.hp < e.maxHp) {
      const barW = r * 2.2;
      const barH = 4;
      const barY = sy - r * squishY - (e.isBoss ? r * 0.55 : (e.isShieldMob ? 35 : 10));
  ...
  _drawHUD(ctx) {
    ...
    // 🛡️ GOLDMIMC — 양쪽 4+4 투명 풍선 자국 (항상 표시)
    this._drawGoldmimicBalloons(ctx);
  }

  // ══════════════════════════════════════
  //  🛡️ GOLDMIMC 풍선 HUD + 완성 연출
  // ══════════════════════════════════════

  /** 양쪽 4+4 투명 풍선 자국 그리기 */
  _drawGoldmimicBalloons(ctx) {
    const word = this._goldmimicWord; // ['G','O','L','D','M','I','M','C']
    const W = this.W, H = this.H;

    // 풍선 크기/위치 설정
    const balloonR = 14;              // 풍선 반지름
    const gap = 54;                   // 세로 간격
    const startY = H * 0.18;         // 시작 Y (위로 이동)
    const leftX = 22;                 // 왼쪽 X
    const rightX = W - 22;           // 오른쪽 X
    const t = Date.now() * 0.001;    // 시간 기반 애니메이션
    ...
    for (let i = 0; i < 8; i++) {
      const isLeft = i < 4;
      const idx = isLeft ? i : i;     // 순서대로 0~7
      const localIdx = isLeft ? i : i - 4;
      const bx = isLeft ? leftX : rightX;
      const by = startY + localIdx * gap;

      // 미세한 떠다니는 움직임
      const floatY = Math.sin(t * 0.8 + i * 0.7) * 2;
      const floatX = Math.sin(t * 0.5 + i * 1.1) * 1;
      const fx = bx + floatX;
      const fy = by + floatY;

      const collected = this._collectedSlots[idx];
      ...
      if (collected) {
        ...
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(fx, fy + balloonR + 2);
        ctx.lineTo(fx + Math.sin(t + i) * 2, fy + balloonR + 16);
        ctx.stroke();

        // 풍선 몸체 — 금색 글로우
        ctx.shadowColor = '#FFD700';
        ctx.shadowBlur = 8 + Math.sin(t * 2 + i) * 3;
        ctx.fillStyle = 'rgba(255,215,0,0.35)';
        ctx.beginPath();
        ctx.ellipse(fx, fy, balloonR, balloonR * 1.15, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.shadowBlur = 0;
        ...
        ctx.shadowColor = '#FFD700';
        ctx.shadowBlur = 6;
        ctx.fillText(word[idx], fx, fy);
        ctx.shadowBlur = 0;

        // 하이라이트 반짝임
        ctx.fillStyle = 'rgba(255,255,255,0.3)';
        ctx.beginPath();
        ctx.ellipse(fx - balloonR * 0.25, fy - balloonR * 0.3, balloonR * 0.2, balloonR * 0.15, -0.3, 0, Math.PI * 2);
        ctx.fill();
      } else {
        // ── 미수집: 아주 희미한 투명 풍선 자국 ──
        // 줄 (거의 안 보임)
  ...
  /** 8글자 완성 → 네온사인 연출 시작 */
  _startMimicCompleteSequence() {
    // 풍선 즉시 리셋 (연출 시작과 동시에)
    this._collectedSlots = new Array(8).fill(false);
    this._collectedCount = 0;
    GameState.goldmimicSlots = [...this._collectedSlots];

    this._mimicCompleteAnim = {
      phase: 'neon',       // neon → giant → eat → transition
      timer: 0,
      neonDuration: 2500,  // 네온사인 빛남 + 회전 2.5초
      giantDuration: 2000, // 거대 미믹 출현 2초
      eatDuration: 1500,   // 잡아먹는 연출 1.5초
      fadeDuration: 1000,  // 페이드 전환 1초
    };
    this._screenShake = { timer: 0, duration: 800, intensity: 5 };
  }

  /** 매 프레임 미믹 완성 연출 업데이트 */
  _updateMimicSequence(dt) {
    const anim = this._mimicCompleteAnim;
    if (!anim) return;
    anim.timer += dt;

    switch (anim.phase) {
      case 'neon':
        if (anim.timer >= anim.neonDuration) {
          anim.phase = 'giant';
          anim.timer = 0;
          this._screenShake = { timer: 0, duration: 500, intensity: 8 };
        }
        break;
      case 'giant':
        if (anim.timer >= anim.giantDuration) {
          anim.phase = 'eat';
          anim.timer = 0;
        }
        break;
      case 'eat':
        if (anim.timer >= anim.eatDuration) {
          anim.phase = 'transition';
          anim.timer = 0;
        }
        break;
      case 'transition':
        if (anim.timer >= anim.fadeDuration) {
          // 전투 일시정지 → 미믹 뱃속 미니게임 진입
          this._enterMimicBelly();
          this._mimicCompleteAnim = null;
        }
        break;
    }
  }

  /** 미믹 완성 연출 렌더링 (HUD 위 오버레이) */
  _drawMimicSequence(ctx) {
    const anim = this._mimicCompleteAnim;
    if (!anim) return;

    const W = this.W, H = this.H;
    const t = Date.now() * 0.001;
    const word = this._goldmimicWord;

    ctx.save();

    if (anim.phase === 'neon') {
      // ── 네온사인: 글자가 중앙으로 모여 빛나며 회전 ──
      const progress = Math.min(anim.timer / anim.neonDuration, 1);
      const centerX = W / 2;
      ...
      for (let i = 0; i < 8; i++) {
        ...
        const radius = baseRadius * (1 - progress * 0.4); // 점점 모임
        const lx = centerX + Math.cos(angle) * radius;
        const ly = centerY + Math.sin(angle) * radius * 0.6;

        // 네온 글로우 (점점 강해짐)
        const glow = 5 + progress * 20 + Math.sin(t * 5 + i) * 5;
        const neonColors = ['#FFD700', '#FF6B6B', '#4488FF', '#44FF88', '#FF44FF', '#44FFFF', '#FF8800', '#FFFF44'];
        const color = neonColors[i % neonColors.length];

        ctx.font = `bold ${18 + progress * 10}px monospace`;
      ...
      if (progress > 0.6) {
        ...
        ctx.shadowColor = '#FFD700';
        ctx.shadowBlur = 15;
        ctx.fillText('✨ GOLD MIMIC! ✨', centerX, centerY + baseRadius * 0.8);
        ctx.shadowBlur = 0;
        ctx.globalAlpha = 1;
      }
    }
    ...
    if (anim.phase === 'giant') {
      // ── 거대 황금 미믹 출현 (화면 1/3 크기) ──
      const progress = Math.min(anim.timer / anim.giantDuration, 1);
      const centerX = W / 2;
      const centerY = H * 0.45;
      const mimicSize = (H / 3) * progress; // 점점 커짐
      ...
      // 거대 미믹 몸체 (금색 원)
      const pulse = Math.sin(t * 4) * 0.05;
      const bodyR = mimicSize * 0.5 * (1 + pulse);
      ...
      const bodyGrad = ctx.createRadialGradient(
        centerX - bodyR * 0.2, centerY - bodyR * 0.2, bodyR * 0.1,
        centerX, centerY, bodyR
      );
      bodyGrad.addColorStop(0, '#FFEC8B');
      bodyGrad.addColorStop(0.5, '#FFD700');
      bodyGrad.addColorStop(1, '#B8860B');
      ctx.fillStyle = bodyGrad;
      ctx.beginPath();
      ctx.ellipse(centerX, centerY, bodyR, bodyR * 0.85, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.shadowBlur = 0;
      ...
      // 눈 (큰 눈)
      const eyeR = bodyR * 0.15;
      const eyeY = centerY - bodyR * 0.15;
      // 흰자
      ctx.fillStyle = '#fff';
      ctx.beginPath();
      ctx.ellipse(centerX - bodyR * 0.22, eyeY, eyeR, eyeR * 1.2, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.ellipse(centerX + bodyR * 0.22, eyeY, eyeR, eyeR * 1.2, 0, 0, Math.PI * 2);
      ctx.fill();
      ...
      ctx.fillStyle = '#1a1a2e';
      ctx.beginPath();
      ctx.arc(centerX - bodyR * 0.22, eyeY + eyeR * 0.1, eyeR * 0.55, 0, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.arc(centerX + bodyR * 0.22, eyeY + eyeR * 0.1, eyeR * 0.55, 0, Math.PI * 2);
      ctx.fill();
      ...
      ctx.fillStyle = '#fff';
      ctx.beginPath();
      ctx.arc(centerX - bodyR * 0.25, eyeY - eyeR * 0.2, eyeR * 0.2, 0, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.arc(centerX + bodyR * 0.19, eyeY - eyeR * 0.2, eyeR * 0.2, 0, Math.PI * 2);
      ctx.fill();

      // 입 (크게 벌리는 중 — 나중에 잡아먹기 위해)
      const mouthOpen = progress * 0.6;
      ctx.fillStyle = '#2a1a0a';
      ctx.beginPath();
      ctx.ellipse(centerX, centerY + bodyR * 0.25, bodyR * 0.35, bodyR * mouthOpen * 0.25, 0, 0, Math.PI * 2);
      ctx.fill();

      // 주변 코인 파티클
      for (let i = 0; i < 6; i++) {
        const angle = t * 2 + i * (Math.PI / 3);
        const orbitR = bodyR * 1.4;
        const cx2 = centerX + Math.cos(angle) * orbitR;
        const cy2 = centerY + Math.sin(angle) * orbitR * 0.5;
        ctx.font = `${Math.round(mimicSize * 0.08)}px serif`;
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
    ...
    if (anim.phase === 'eat') {
      // ── 영웅을 잡아먹는 연출 ──
      const progress = Math.min(anim.timer / anim.eatDuration, 1);
      const centerX = W / 2;
      const centerY = H * 0.45;
      const bodyR = (H / 3) * 0.5;
      ...
      // 큰 입 (점점 더 크게)
      const mouthH = bodyR * (0.3 + progress * 0.4);
      ctx.fillStyle = '#1a0a00';
      ctx.beginPath();
      ctx.ellipse(centerX, centerY + bodyR * 0.2, bodyR * 0.4, mouthH, 0, 0, Math.PI * 2);
      ctx.fill();
      // 입 안쪽 깊이감
      ctx.fillStyle = '#0a0500';
      ctx.beginPath();
      ctx.ellipse(centerX, centerY + bodyR * 0.25, bodyR * 0.25, mouthH * 0.6, 0, 0, Math.PI * 2);
      ctx.fill();
      ...
      // 왼눈 >
      ctx.beginPath();
      ctx.moveTo(centerX - bodyR * 0.32, eyeY - 5);
      ctx.lineTo(centerX - bodyR * 0.22, eyeY);
      ctx.lineTo(centerX - bodyR * 0.32, eyeY + 5);
      ctx.stroke();
      // 오른눈 >
      ctx.beginPath();
      ctx.moveTo(centerX + bodyR * 0.12, eyeY - 5);
      ctx.lineTo(centerX + bodyR * 0.22, eyeY);
      ctx.lineTo(centerX + bodyR * 0.12, eyeY + 5);
      ctx.stroke();

      // 영웅이 빨려들어감 (아래에서 입 안으로)
      const heroScale = 1 - progress;
      const heroX = centerX;
      const heroY = centerY + bodyR * 0.2 + (1 - progress) * bodyR * 0.8;
      if (heroScale > 0.05) {
        ctx.globalAlpha = heroScale;
        ctx.font = `${Math.round(24 * heroScale)}px serif`;
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
      ...
      if (progress > 0.5) {
        ...
        ctx.shadowBlur = 10;
        ctx.textAlign = 'center';
        ctx.fillText('냠냠냠...! 🍽️', centerX, centerY - bodyR - 20);
        ctx.shadowBlur = 0;
      }
    }

    if (anim.phase === 'transition') {
      // ── 페이드 전환 (검은 화면으로) ──
      const progress = Math.min(anim.timer / anim.fadeDuration, 1);
  ...
  // ══════════════════════════════════════
  //  공중전 Phase 2 — 보스 분노 비행전
  // ══════════════════════════════════════
  _startAerialPhase2(boss, bossMaxHp) {
    this._combatPausedForAerial = true;
    // 다크 오리진 맵 생성 (dark_forest 테마)
    const darkMap = generateMap('dark_forest', 60, 30);
    this._aerialPhase2 = new AerialBossPhase2(this.canvas, {
      boss,
      bossMaxHp,
      player: this.player,
      sharedKeys: this._keys,
      sharedTouchDir: this._touchDir,
      stageLevel: this.stageLevel,
      rageSystem: this.rageSystem,
      map: darkMap,
      renderMapFn: renderMap,
      onVictory: () => this._exitAerialPhase2('victory'),
      onDeath: () => this._exitAerialPhase2('death'),
    });
    this._aerialPhase2.start();
  }

  _exitAerialPhase2(result) {
    if (this._aerialPhase2) {
      this._aerialPhase2.destroy();
      this._aerialPhase2 = null;
    }
    this._combatPausedForAerial = false;
    if (result === 'death') {
      this.onDeath();
  ...
  /** 미믹 뱃속 미니게임 진입 */
  _enterMimicBelly() {
    // (리셋은 _startMimicCompleteSequence에서 이미 완료)

    // 전투 일시정지
    this._combatPausedForBelly = true;

    // 미믹 뱃속 미니게임 시작 (키 입력 공유)
    this._mimicBelly = new GoldenMimicBelly(this.canvas, {
      stageLevel: this.stageLevel,
      sharedKeys: this._keys,
      sharedTouchDir: this._touchDir,
      playerStats: {
      ...
      onComplete: (rewards) => {
        this._exitMimicBelly(rewards);
      },
    });
    this._mimicBelly.start();
  }

  /** 미믹 뱃속에서 나옴 */
  _exitMimicBelly(rewards) {
    // 보상 적용
    if (rewards) {
      if (rewards.gold) {
        GameState.addGold(rewards.gold);
        this.totalGold += rewards.gold;
      }
      if (rewards.upgrades) {
        for (const upg of rewards.upgrades) {
          const item = UPGRADE_ITEMS.find(u => u.id === upg);
          if (item) item.apply(this.player, this);
        }
      }
      if (rewards.hp) {
        this.healPlayer(rewards.hp);
      }
    }

    this._mimicBelly = null;
    this._combatPausedForBelly = false;

    // 복귀 연출: 화면 흔들림 + 파티클
    this._screenShake = { timer: 0, duration: 400, intensity: 5 };
    for (let i = 0; i < 25; i++) {
      this.particles.push({
        x: this.player.x, y: this.player.y,
        vx: (Math.random() - 0.5) * 6, vy: (Math.random() - 0.5) * 6,
        life: 1000, color: i % 3 === 0 ? '#FFD700' : i % 3 === 1 ? '#FFA500' : '#FFEC8B',
        size: 3 + Math.random() * 3, type: 'circle',
      });
    }
  }
