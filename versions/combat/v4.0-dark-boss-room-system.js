...
// ★ 보스 Y 오프셋 — 양수=위로, 음수=아래로 (1칸 = 40px)
const BOSS_Y_OFFSET = 120;
...
export default class BossRoomSystem {
  ...
  enterBossRoom() {
    ...
    const bossY = player.y - this.engine.H * 0.4 - BOSS_Y_OFFSET;
    if (bossDef) {
  ...
  _updateBossCombat(dt) {
    ...
    if (this.config.phaseTransition &&
        ...
        this.config.combatMode !== 'aerial_phase2') {
  ...
  _triggerAerialTransition() {
    this.config.combatMode = 'aerial_phase2';
    // AerialBossPhase2 — 비행기 공중전 스타일 (Phase 2)
    if (this.engine._startAerialPhase2) {
      this.engine._startAerialPhase2(this.boss, this.bossMaxHp);
    }
  }
  ...
  activateAtPosition(arenaX, arenaY, theme) {
    ...
    // 보스 생성 — 아레나 로컬 좌표 사용 (월드좌표 X, 아레나 내부 배치)
    const player = this.engine.player;
    const bossType = this.config.bossType;
    const bossDef = ARENA_BOSSES[bossType];
    const aw = arenaConf.width;
    const ah = arenaConf.height;
    // 보스: 아레나 상단 30% 중앙
    const bossX = aw * 0.5;
    const bossY = ah * 0.25;
    ...
    // 플레이어를 아레나 하단 70% 중앙으로 재배치
    player.x = aw * 0.5;
    player.y = ah * 0.7;

    // 카메라를 아레나 좌표로 즉시 이동 (부드러운 전환 대신 즉시)
    const cam = this.engine.camera;
    cam.x = Math.max(0, player.x - this.engine.W * 0.5);
    cam.y = Math.max(0, player.y - this.engine.H * 0.5);
