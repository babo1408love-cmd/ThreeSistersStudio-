...
export default class CombatEngine {
  ...
  _updateEnemies(dt) {
    this.enemies.forEach(e => {
      ...
      // ══════════════════════════════════════
      //  🛡️ 수비대 시스템 — 보스를 향해 전진하는 영웅 vs 길을 막는 몹
      //  세계관: 몹은 버섯돌이 대마왕의 수비대.
      //         영웅 전방(위쪽)에 방어선을 형성하고 길을 막는다.
      //         영웅이 접근하면 교전 모드로 전환, 밀어내려 한다.
      // ══════════════════════════════════════
      const dx = this.player.x - e.x;
      const dy = this.player.y - e.y;
      const dist = Math.sqrt(dx * dx + dy * dy);
      const contactDist = (e.radius || 14) + this.player.radius;
      const engageRange = 120; // 교전 전환 거리

      if (dist < engageRange) {
        // 🗡️ 교전 모드: 영웅이 가까이 오면 맞서 싸움
        const pushSpd = e.fixedSpeedMul
          ? this.player.speed * e.fixedSpeedMul * (dt / 16)
          : this.player.speed * 0.7 * (dt / 16);
        if (dist > contactDist) {
          let spd = pushSpd;
          if (spd > dist - contactDist) spd = dist - contactDist;
          e.x += (dx / dist) * spd;
          e.y += (dy / dist) * spd;
        }
      } else {
        // 🛡️ 수비 모드: 방어선 유지, 천천히 아래로 전진해서 길 차단
        const guardSpd = e.fixedSpeedMul
          ? this.player.speed * e.fixedSpeedMul * (dt / 16)
          : this.player.speed * 0.35 * (dt / 16);
        e.y += guardSpd;
        // 좌우 흔들림 (방어 자세)
        e.x += Math.sin(e.bobPhase * 1.5) * 0.3;
      }
      ...
      // 영웅 뒤(아래)로 밀려난 수비대 → 전방 재배치
      if (e.y > this.player.y + this.H * 0.5) {
        e.y = this.player.y - this.H * (0.5 + Math.random() * 0.3);
        e.x = Math.random() < 0.5
          ? this.map.mapW * 0.25 + (Math.random() - 0.5) * 80
          : this.map.mapW * 0.75 + (Math.random() - 0.5) * 80;
      }

      // Contact damage — 수비대가 영웅과 부딪히면 저항 데미지
      if (!e.passive) {
  ...
  _updateCamera() {
    // 종스크롤: 플레이어를 화면 하단 10%에 배치 → 전방(위) 90% 시야 확보
