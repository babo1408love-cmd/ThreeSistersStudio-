...
export default class CombatEngine {
  ...
  _updateSlotHeroes(dt) {
    ...
    this.slotHeroes.forEach((h, i) => {
      ...
      // Auto-attack — 적 없어도 항상 발사, 유도탄 획득 시 추적
      h.atkTimer -= dt;
      if (h.atkTimer <= 0) {
        const hNearest = this.enemies.length > 0 ? this._findNearest(h, this.enemies) : null;
        const useH = this._homingUnlocked && hNearest;
        const hAngle = useH ? Math.atan2(hNearest.y - h.y, hNearest.x - h.x) : -Math.PI / 2;
  ...
  _updateSpirits(dt) {
    ...
    this.spirits.forEach(s => {
      ...
      // 정령 미사일 발사 — 적 없어도 항상 발사, 유도탄 시 추적
      s.atkTimer -= dt;
      if (s.atkTimer <= 0) {
        ...
        const useS = this._homingUnlocked && sNearest;
        const sAngle = useS ? Math.atan2(sNearest.y - s.y, sNearest.x - s.x) : -Math.PI / 2;
  ...
  _updateAutoAttack(dt) {
    ...
    if (this.player.atkTimer <= 0) {
      const nearest = this.enemies.length > 0
        ? (bossTarget || this._findNearest(this.player, this.enemies))
        : null;
      ...
      for (let i = 0; i < this.player.shotCount; i++) {
        ...
        this.projectiles.push({
          ...
          emoji: isCrit ? '💥' : this.rageSystem.isActive() ? '💢' : '⚡',
          pierce: this.player.pierce,
          homing: !!this.player.homing,
          target: useHoming ? nearest : null,
      ...
      if (isCrit) {
        this.particles.push({
          ...
          text: '💥CRIT!', color: '#FF00FF', type: 'text',
          life: 600, vy: -1.5, vx: 0,
        });
      }
  ...
  _damagePlayer(damage) {
    ...
    if (this.player._shieldHp > 0) {
      ...
      this.particles.push({
        x: this.player.x, y: this.player.y - 20,
        text: `🛡️-${Math.round(absorbed)}`, color: '#66CCFF', type: 'text',
        life: 600, vy: -1, vx: 0,
      });
