...
export default class CombatEngine {
  ...
  _updateEnemies(dt) {
    this.enemies.forEach(e => {
      ...
      // ── 디펜스 모드: 레인 행진 (위→아래) ──
      // 몹은 플레이어를 추적하지 않고 자기 레인을 따라 아래로 행진
      const marchSpd = e.fixedSpeedMul
        ? this.player.speed * e.fixedSpeedMul * (dt / 16)
        : this.player.speed * 0.9 * (dt / 16);
      e.y += marchSpd;  // 아래로 행진 (Y 증가)

      // 맵 아래쪽 끝을 넘으면 제거 (방어선 돌파)
      if (e.y > this.player.y + this.H * 0.6) {
        // 돌파한 몹은 플레이어에게 데미지 + 제거
        if (!e.passive) {
          this._damagePlayer(e.attack || e.atk || 5);
          this.particles.push({
            x: e.x, y: e.y - 10,
            text: '💥 돌파!', color: '#ff4444', type: 'text',
            life: 1000, vy: -1, vx: 0,
          });
        }
        this.heroEngine.removeMob(e);
        e.hp = 0; // 제거 마킹
        return;
      }

      // Clamp to map bounds (좌우만)
      e.x = Math.max(0, Math.min(this.map.mapW, e.x));

      // Contact damage — 몹이 플레이어 접촉 시 (레인 지나가다 부딪힘)
