        ...
        : candidates.map(item => {
          ...
          return `<div class="hs-popup-item" data-uid="${uid}">
            ...
            <div class="hs-popup-item-info">
              <div style="font-weight:700;">${item.name} <span style="color:${rc};font-size:0.8em;">${RARITY_NAMES[item.rarity] || ''}</span></div>
              <div style="font-size:0.8em;color:var(--text-secondary);">${_formatStats(item.stats)}</div>
            </div>
      ...
      ${available.length === 0
        ...
        : available.map(hero => {
          ...
          return `<div class="hs-popup-item${check.allowed ? '' : ' disabled-row'}">
            ...
            <div class="hs-popup-item-info">
              <div style="font-weight:700;">${hero.name} <span style="color:${rc};font-size:0.8em;">${RARITY_NAMES[hero.rarity] || ''}</span></div>
              <div style="font-size:0.8em;color:var(--text-secondary);">${attrInfo.emoji || ''} ${attrInfo.name || ''} | ${hero.passiveSkill?.name || ''}</div>
              ${!check.allowed ? `<div style="font-size:0.75em;color:#ff6b6b;">${check.reason}</div>` : ''}
            </div>
    ...
    <div class="hs-popup-list">
      ${PET_ROSTER.map(pet => {
        ...
        return `<div class="hs-popup-item${isCur ? ' current-class' : ''}">
          ...
          <div class="hs-popup-item-info">
            <div style="font-weight:700;">${pet.name} <span style="color:${rc};font-size:0.8em;">${RARITY_NAMES[pet.rarity] || ''}</span></div>
            <div style="font-size:0.8em;color:var(--text-secondary);">${attrInfo.emoji || ''} ${attrInfo.name || ''} | ${pet.healSkill?.name || ''}</div>
            <div style="font-size:0.75em;color:var(--text-muted);">${pet.healSkill?.description || ''}</div>
          </div>
          ${!isCur ? `<button class="btn btn-sm btn-primary hs-pick-pet" data-key="${pet.key}">배치</button>` : '<span style="font-size:0.75em;color:#67e8f9;">현재</span>'}
        </div>`;
      }).join('')}
    </div>
  `, (popup) => {
      ...
      ${Object.values(HERO_CLASSES).map(cls => {
        ...
        return `<div class="hs-popup-item${isCur ? ' current-class' : ''}">
          ...
          <div class="hs-popup-item-info">
            ...
            <div style="font-size:0.8em;color:var(--text-secondary);">레벨당: ${growthText}</div>
          </div>
...
function _showSubPopup(html, bindFn) {
  ...
  document.body.appendChild(subPopupEl);
...
function _injectStyles() {
  ...
  style.textContent = `
    .hero-screen {
      min-width: 330px; max-width: 480px; max-height: 85vh; overflow-y: auto;
      padding: 16px; scrollbar-width: thin;
    }
    .hs-header {
      display: flex; justify-content: space-between; align-items: center; margin-bottom: 14px;
    }
    .hs-title { font-size: 1.3em; font-weight: 700; }

    /* 초상화 카드 */
    .hs-portrait-card {
      display: flex; gap: 14px; align-items: center;
      padding: 14px; background: var(--bg-card, rgba(30,35,55,0.9));
      border-radius: 14px; margin-bottom: 14px;
      border: 1px solid rgba(255,255,255,0.08);
    }
    .hs-portrait { font-size: 2.8em; }
    .hs-basic { flex: 1; }
    .hs-name { font-size: 1.1em; font-weight: 700; margin-bottom: 4px; }
    .hs-class-row { font-size: 0.85em; color: var(--text-secondary, #aaa); margin-bottom: 6px; display: flex; align-items: center; gap: 4px; flex-wrap: wrap; }
    .hs-exp-row { display: flex; align-items: center; gap: 8px; }
    .hs-exp-bar { flex: 1; height: 10px; background: rgba(255,255,255,0.08); border-radius: 5px; overflow: hidden; }
    .hs-exp-fill { height: 100%; background: linear-gradient(90deg, #67e8f9, #9b8aff); border-radius: 5px; transition: width 0.3s; }
    .hs-exp-text { font-size: 0.75em; color: var(--text-secondary, #aaa); white-space: nowrap; }

    /* 섹션 */
    .hs-section { margin-bottom: 14px; }
    .hs-section-title {
      font-size: 0.95em; font-weight: 700; margin-bottom: 8px;
      display: flex; justify-content: space-between; align-items: center;
    }
    .hs-points { font-size: 0.8em; color: #fbbf24; font-weight: 400; }

    /* 스탯 그리드 */
    .hs-stats-grid {
      display: grid; grid-template-columns: repeat(4, 1fr); gap: 6px;
      padding: 10px; background: var(--bg-card, rgba(30,35,55,0.9));
      border-radius: 10px; border: 1px solid rgba(255,255,255,0.06);
    }
    .hs-stat-cell { text-align: center; font-size: 0.8em; }
    .hs-stat-icon { display: block; font-size: 1.3em; margin-bottom: 2px; }
    .hs-stat-label { display: block; color: var(--text-muted, #888); font-size: 0.85em; }
    .hs-stat-val { display: block; font-weight: 700; color: #ddd; }

    /* 장비 그리드 */
    .hs-equip-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; }
    .hs-equip-slot {
    ...
    .hs-equip-emoji { font-size: 1.6em; margin-bottom: 2px; }
    .hs-equip-name { font-size: 0.8em; font-weight: 600; }
    .hs-equip-rarity { font-size: 0.7em; }
    .hs-equip-empty { font-size: 0.7em; color: var(--text-muted, #888); }

    /* 영웅 슬롯 */
    .hs-hero-slots { display: flex; gap: 8px; flex-wrap: wrap; }
    .hs-hero-slot {
      flex: 1 1 55px; min-width: 55px; max-width: 70px;
    ...
    .hs-hero-emoji { font-size: 1.5em; }
    .hs-hero-label { font-size: 0.7em; margin-top: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }

    /* 펫 영역 */
    .hs-pet-area { display: flex; gap: 10px; align-items: flex-start; flex-wrap: wrap; }
    .hs-pet-slot {
      ...
      border-radius: 12px; transition: all 0.2s; min-width: 80px;
    }
    ...
    .hs-pet-emoji { font-size: 2em; }
    .hs-pet-label { font-size: 0.8em; font-weight: 600; margin-top: 2px; }
    .hs-pet-sub { font-size: 0.65em; color: var(--text-muted, #888); margin-top: 2px; }
    .hs-pet-equip-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 6px; flex: 1; min-width: 160px; }
    .hs-pet-equip-slot {
      text-align: center; padding: 8px 4px; cursor: pointer;
    ...
    .hs-pet-equip-emoji { font-size: 1.3em; }
    .hs-pet-equip-name { font-size: 0.75em; font-weight: 600; }
    .hs-pet-equip-rarity { font-size: 0.65em; }
    .hs-pet-equip-hint { font-size: 0.65em; color: var(--text-muted, #888); }
    .hs-pet-equip-empty { font-size: 0.8em; color: var(--text-muted, #888); padding: 10px; flex: 1; }

    /* 업그레이드 그리드 */
    .hs-upgrade-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 8px; }
    .hs-upgrade-card {
      padding: 8px 10px; background: var(--bg-card, rgba(30,35,55,0.9));
      border: 1px solid rgba(255,255,255,0.06); border-radius: 10px;
    }
    .hs-upgrade-top { display: flex; justify-content: space-between; font-size: 0.85em; margin-bottom: 4px; }
    ...
    .hs-upgrade-next { font-size: 0.7em; color: #fbbf24; margin-bottom: 4px; }
    .hs-upgrade-btn { font-size: 0.75em; padding: 2px 8px; }
    .hs-upgrade-btn.disabled { opacity: 0.3; cursor: default; }

    /* 스킬 */
    .hs-skills { display: flex; flex-direction: column; gap: 8px; }
    .hs-skill-card {
      padding: 10px 12px; background: var(--bg-card, rgba(30,35,55,0.9));
      border: 1px solid rgba(255,255,255,0.06); border-radius: 10px; font-size: 0.85em;
    }
    .hs-skill-type {
      font-size: 0.7em; padding: 1px 6px; border-radius: 4px; margin-right: 6px; font-weight: 700;
    }
    ...
    .hs-skill-desc { font-size: 0.85em; color: var(--text-secondary, #aaa); margin-top: 4px; }
    ...
    .hs-sub-popup {
      background: #1a1a2e; border-radius: 16px; padding: 18px;
      max-width: 380px; width: 90%; max-height: 75vh; overflow-y: auto;
      border: 1px solid rgba(255,255,255,0.1);
    }
    .hs-popup-title { font-size: 1.1em; font-weight: 700; margin-bottom: 12px; }
    .hs-popup-current {
      display: flex; justify-content: space-between; align-items: center;
      padding: 8px 10px; background: rgba(255,255,255,0.04); border-radius: 8px; margin-bottom: 10px; font-size: 0.9em;
    }
    .hs-popup-list { display: flex; flex-direction: column; gap: 6px; max-height: 300px; overflow-y: auto; }
    .hs-popup-item {
      display: flex; align-items: center; gap: 10px; padding: 8px 10px;
      background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.06);
      border-radius: 8px; font-size: 0.85em; transition: background 0.15s;
    }
    ...
    .hs-popup-item-emoji {
      font-size: 1.5em; width: 38px; height: 38px; display: flex; align-items: center; justify-content: center;
    ...
    .hs-toast {
      ...
      background: rgba(30,35,55,0.95); color: #fbbf24; padding: 8px 20px;
      border-radius: 20px; font-size: 0.85em; z-index: 7000;
    ...
    .hs-tut-step { font-size: 0.75em; color: #67e8f9; margin-bottom: 6px; }
    .hs-tut-title { font-size: 1.1em; font-weight: 700; margin-bottom: 8px; }
    .hs-tut-desc { font-size: 0.9em; color: #ccc; margin-bottom: 14px; line-height: 1.5; }
    .hs-tut-buttons { display: flex; justify-content: space-between; gap: 10px; }
    ...
    /* 작은 버튼 */
    .btn-tiny {
      font-size: 0.75em; padding: 2px 8px; border: 1px solid rgba(255,255,255,0.15);
