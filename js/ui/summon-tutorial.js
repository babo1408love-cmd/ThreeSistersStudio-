...
export function showSummonTutorial(onComplete) {
  ...
  function renderStep() {
    ...
    card.style.cssText = `
      background:var(--bg-card,#1e2337);border-radius:16px;padding:24px;
      max-width:360px;width:90%;text-align:center;border:2px solid var(--border-subtle,#333);
    `;

    // Title
    card.innerHTML = `
      <div style="font-size:40px;margin-bottom:8px;">${step.emoji}</div>
      <div style="font-size:1.1em;font-weight:700;color:var(--gold,#fbbf24);margin-bottom:12px;">${step.title}</div>
    `;

    // Lines
    for (const line of step.lines) {
      const p = document.createElement('div');
      p.style.cssText = 'font-size:0.9em;color:var(--text-secondary,#aaa);margin-bottom:6px;text-align:left;padding:0 8px;';
      p.textContent = line;
      card.appendChild(p);
    }
    ...
    if (step.visual) {
      ...
      vis.style.cssText = 'font-size:0.85em;color:var(--green,#86efac);margin:12px 0;padding:8px;background:rgba(134,239,172,0.1);border-radius:8px;';
    ...
    if (step.table) {
      ...
      pre.style.cssText = 'font-size:0.7em;color:var(--text-secondary,#aaa);text-align:left;margin:12px 0;line-height:1.4;overflow-x:auto;';
    ...
    if (step.closingLine) {
      ...
      cl.style.cssText = 'font-size:0.9em;color:var(--gold,#fbbf24);margin:8px 0;font-weight:700;';
    ...
    progress.style.cssText = 'font-size:0.75em;color:var(--text-muted,#666);margin:12px 0 8px;';
    ...
    skip.style.cssText = 'margin-top:8px;margin-left:8px;font-size:0.8em;';
    skip.onclick = () => {
      overlay.remove();
      GameState._summonTutorialSeen = true;
      if (onComplete) onComplete();
